...
public class ProductoCTO extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        switch(accion){
            case "listar_datos":
                ArrayList<ProductoDTO> lista=fcd.listarProductos();
                request.setAttribute("lista_prod", lista);
                request.getRequestDispatcher("ProductoVTA.jsp").forward(request, response);
                break;
            case "eliminar":
                ProductoDTO elim=new ProductoDTO();
                elim.setId_prod(Integer.parseInt(request.getParameter("id")));
                boolean aux=fcd.borrar(elim);
